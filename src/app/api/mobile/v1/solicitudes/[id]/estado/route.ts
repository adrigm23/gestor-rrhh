...
// Igual que actualizarSolicitud en la web: solo GERENTE/ADMIN_SISTEMA.
export const POST = withMobileCors(async (request: Request, context: RouteContext) => {
  ...
  const limit = await rateLimitService.check(
    `solicitudes:estado:${auth.context.userId}`,
    WRITE_LIMIT,
  ...
  const parsed = UpdateSolicitudEstadoRequestSchema.safeParse(body);
  if (!parsed.success) {
    return badRequest("Estado inválido.");
  }

  try {
    const result = await solicitudService.updateEstado(auth.context.userId, auth.context.role, id, parsed.data.estado);
    return NextResponse.json<UpdateSolicitudEstadoResponse>(result);
  } catch (error) {
    console.error("[POST /api/mobile/v1/solicitudes/[id]/estado]", error);
    return serverError();
  }
});
