...
function toDto(
  entry: Awaited<ReturnType<typeof solicitudService.listPendingForManager>>[number],
): SolicitudManagerDto {
  return {
    id: entry.id,
    tipo: entry.tipo,
...
// Igual que vacaciones-ausencias/page.tsx: solo GERENTE/ADMIN_SISTEMA.
export const GET = withMobileCors(async (request: Request) => {
  ...
  const limit = await rateLimitService.check(
    `solicitudes:pendientes:${auth.context.userId}`,
    LIST_LIMIT,
    LIST_WINDOW_SECONDS,
  );
  if (!limit.allowed) {
  ...
  try {
    const solicitudes = await solicitudService.listPendingForManager(auth.context.userId, auth.context.role, 30);
    return NextResponse.json<ListSolicitudesManagerResponse>({ solicitudes: solicitudes.map(toDto) });
  } catch (error) {
    console.error("[GET /api/mobile/v1/solicitudes/pendientes]", error);
    return serverError();
  }
});
