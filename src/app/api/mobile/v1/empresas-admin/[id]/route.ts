...
import type { ApiErrorResponse, EliminarEmpresaResponse } from "@gestor-rrhh/shared";
...
// Igual que eliminarEmpresa en la web: exclusivo ADMIN_SISTEMA.
export const DELETE = withMobileCors(async (request: Request, context: RouteContext) => {
  ...
  const limit = await rateLimitService.check(
    `empresas-admin:delete:${auth.context.userId}`,
    WRITE_LIMIT,
  ...
  try {
    const result = await empresaService.eliminarEmpresa(id);
    return NextResponse.json<EliminarEmpresaResponse>(result);
  } catch (error) {
    console.error("[DELETE /api/mobile/v1/empresas-admin/[id]]", error);
    return serverError();
  }
});
