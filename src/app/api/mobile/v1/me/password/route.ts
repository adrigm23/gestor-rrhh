...
export const POST = withMobileCors(async (request: Request) => {
  ...
  const limit = await rateLimitService.check(
    `me:password:${auth.context.userId}`,
    WRITE_LIMIT,
  ...
  if (!parsed.success) {
    return badRequest("Datos de contraseña inválidos.");
  }

  try {
    const result = await usuarioService.changePassword(auth.context.userId, parsed.data);
    return NextResponse.json<ChangePasswordResponse>(result);
  } catch (error) {
    console.error("[POST /api/mobile/v1/me/password]", error);
    return serverError();
  }
});
