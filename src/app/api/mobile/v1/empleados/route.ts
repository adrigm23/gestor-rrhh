...
import type { ApiErrorResponse, ListEmpleadosResponse } from "@gestor-rrhh/shared";
...
// Selector mínimo para formularios de gestor/admin (Fase 2.11). No es el
// directorio completo (Fase 2.13).
export const GET = withMobileCors(async (request: Request) => {
  ...
  const limit = await rateLimitService.check(`empleados:list:${auth.context.userId}`, LIST_LIMIT, LIST_WINDOW_SECONDS);
  if (!limit.allowed) {
    return tooManyRequests(limit.retryAfterSeconds);
  }

  try {
    const empleados = await usuarioService.listEmpleados(auth.context.userId, auth.context.role);
    return NextResponse.json<ListEmpleadosResponse>({ empleados });
  } catch (error) {
    console.error("[GET /api/mobile/v1/empleados]", error);
    return serverError();
  }
});
