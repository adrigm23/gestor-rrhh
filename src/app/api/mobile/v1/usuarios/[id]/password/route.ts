...
// Igual que resetUsuarioPassword en la web: solo ADMIN_SISTEMA.
export const POST = withMobileCors(async (request: Request, context: RouteContext) => {
  ...
  const limit = await rateLimitService.check(
    `usuarios:reset-password:${auth.context.userId}`,
    WRITE_LIMIT,
  ...
  const parsed = ResetPasswordAdminRequestSchema.safeParse(body);
  if (!parsed.success) {
    return badRequest("Contraseña inválida.");
  }

  try {
    const result = await usuarioService.resetPassword(id, parsed.data.password);
    return NextResponse.json<AdminTargetResponse>(result);
  } catch (error) {
    console.error("[POST /api/mobile/v1/usuarios/[id]/password]", error);
    return serverError();
  }
});
