...
// Igual que actualizarDniUsuario en la web: solo ADMIN_SISTEMA.
export const POST = withMobileCors(async (request: Request, context: RouteContext) => {
  ...
  const limit = await rateLimitService.check(
    `usuarios:update-dni:${auth.context.userId}`,
    WRITE_LIMIT,
  ...
  const parsed = UpdateDniAdminRequestSchema.safeParse(body);
  if (!parsed.success) {
    return badRequest("DNI/NIE inválido.");
  }

  try {
    const result = await usuarioService.updateDniAdmin(id, parsed.data.dni);
    return NextResponse.json<UpdateDniAdminResponse>(result);
  } catch (error) {
    console.error("[POST /api/mobile/v1/usuarios/[id]/dni]", error);
    return serverError();
  }
});
