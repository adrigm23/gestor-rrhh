...
// Igual que actualizarEstadoUsuario en la web: solo ADMIN_SISTEMA.
export const POST = withMobileCors(async (request: Request, context: RouteContext) => {
  ...
  const limit = await rateLimitService.check(
    `usuarios:update-estado:${auth.context.userId}`,
    WRITE_LIMIT,
  ...
  const parsed = UpdateEstadoRequestSchema.safeParse(body);
  if (!parsed.success) {
    return badRequest("Acción inválida.");
  }

  try {
    const result = await usuarioService.updateEstado(id, auth.context.userId, parsed.data.accion);
    return NextResponse.json<UpdateEstadoResponse>(result);
  } catch (error) {
    console.error("[POST /api/mobile/v1/usuarios/[id]/estado]", error);
    return serverError();
  }
});
